...
export default async function Home({searchParams}: {searchParams: { [key: string]: string | undefined }}) {
    ...
    const selectedPeriodData = shoulderData?.periods.find((p) => p.period === period);
    ...
    return (
        <>
            ...
            <div className="space-y-2">
                {selectedPeriodData ?
                    <ChartWithRankings data={selectedPeriodData} /> : <p className="text-center">表示できるデータがありません!!</p>
                }
            </div>
        </>
    );
