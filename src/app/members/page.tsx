import { tOnesDataByUser } from '@/lib/TypeDeclarations';
...
export default async function Home({searchParams}: {searchParams: { [key: string]: string | undefined }}) {
    ...
    const shoulderData = selectedData?.onesData.find((sd) => sd.shoulder === shoulder);
            ...
            <div className="flex items-center justify-center space-x-2 mb-4">
                <div className="border-yellow-600 border-2 p-2 rounded w-fit">
                    {periodLinks.map((p, pindex) => (
                        <Link key={pindex} href={`/members?period=${p}&shoulder=${shoulder}id=${id}`}
                ...
                <div className="border-yellow-600 border-2 p-2 rounded w-fit">
                    {shoulderLinks.map((s, sindex) => (
                        <Link key={sindex} href={`/members?period=${period}&shoulder=${s}id=${id}`}
